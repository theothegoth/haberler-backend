...
const newsController = {
  ...
  async getNews(req, res) {
    try {
      ...
      // Track article view FIRST (await it so the fetch gets the new count)
      const ipAddress = req.ip || req.connection.remoteAddress;
      await ArticleView.recordView(id, userId, ipAddress).catch(err => 
        console.error('Error recording view:', err)
      );

      // THEN fetch the news (it will have the updated view_count)
  ...
  async likeNews(req, res) {
    try {
      ...
      // *** CACHE INVALIDATION ***
      // Clear cache so next fetch shows correct Like status
      deleteCache(`article:${id}`).catch(err => console.error('Cache invalidation error:', err));
      deleteCachePattern('news:feed:*').catch(err => console.error('Cache invalidation error:', err));
